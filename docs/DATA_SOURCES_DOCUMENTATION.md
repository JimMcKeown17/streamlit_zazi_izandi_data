...
| 2026 ECD Midline | **Database** | Direct SQL on `assessments_2026` + `ecd_midline_helpers_2026.py` | PostgreSQL | Compares ECD baseline survey 805 with midline survey 891 (`survey_id IN (805, 891)`). **Cross-sectional only** — baseline 805 has no participant IDs, so no learner matching; all analytics count raw response rows in both phases. Unique-learner counts are midline-only (891 has participant IDs). Baseline rows have empty `program_name`/`class_name`, so center views are midline-only. Gender exists only at baseline. |
