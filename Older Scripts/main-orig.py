...
from data_processing_orig import process_data, grade1, gradeR
...
baseline_path = "data/Zazi iZandi Children's database (Baseline)7052024.xlsx"
sheet_name_baseline = "ZZ Childrens Database"
midline_path = "data/Zazi iZandi Midline Assessments database (1).xlsx"
...
sessions_path = "data/Zazi iZandi Children's Session Tracker-08062024.xlsx"
...
with col1:
    st.image('Zazi iZandi logo.png', width=250)
with col2:
...
with st.container():
    ...
    st.plotly_chart(mentor_sessions_fig, width='stretch')
    with st.expander('Click to view data'):
        st.dataframe(df)
