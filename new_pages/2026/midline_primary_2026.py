...
def _load_primary_midline_raw():
    try:
        ...
        return helpers.normalize_primary_assessments(df)
...
def apply_filters(df):
    ...
    col_language, col_grade, col_school = st.columns([1, 1, 2])
    ...
    with col_language:
        ...
        selected_language = st.selectbox(
            "Language",
            language_options,
            key="midline_primary_language_filter",
        )
    ...
    with col_grade:
        ...
        selected_grade = st.selectbox(
            ...
            key="midline_primary_grade_filter",
        )

    with col_school:
        school_values = df["program_name"].dropna().astype(str).str.strip()
        school_options = ["All schools"] + sorted([school for school in school_values.unique().tolist() if school])
        selected_school = st.selectbox("School", school_options, key="midline_primary_school_filter")
    ...
    if selected_school != "All schools":
        filtered = filtered[filtered["program_name"] == selected_school]
...
def render_completion_tab(summary, dimension, label, color):
    if summary.empty:
...
def render_midline_completion_section(df):
    ...
    with by_school:
        render_completion_tab(school_summary, "program_name", "School", "#1f5cc4")
    with by_ea:
        render_completion_tab(ea_summary, "collected_by", "EA", "#2ca02c")


def render_phase_score_chart(df):
    st.header("Baseline vs Midline Scores")
    controls_left, controls_right = st.columns([1, 1])
    with controls_left:
        score_label = st.radio("Sub-test", list(SCORE_OPTIONS.keys()), horizontal=True)
    with controls_right:
        agg_label = st.radio("Statistic", ["Mean", "Median"], horizontal=True)
    ...
    with st.expander("View score summary table"):
        st.dataframe(summary.sort_values(["language", "grade"]), use_container_width=True)


def render_matched_gain_section(matched):
...
def render_benchmark_section(df):
    ...
    with col_grade:
        ...
        if not grade_options:
            ...
            return
        selected_grade = st.selectbox("Benchmark grade", grade_options, index=0)
    with col_threshold:
        default_threshold = 40 if selected_grade == "Grade 1" else 20
        threshold = st.slider("Letters correct threshold", min_value=0, max_value=60, value=default_threshold, step=5)
...
def render_school_section(matched):
    ...
    min_learners = st.slider("Minimum matched learners per school/grade/language", 1, max_min, min(5, max_min))
...
def render_data_quality(df, matched):
    ...
    with st.expander("Unmatched midline learners"):
        ...
        st.dataframe(unmatched[cols], use_container_width=True)
    ...
    with st.expander("Matched learner export"):
        ...
        st.download_button(
            ...
            file_name="2026_primary_midline_matched_learners.csv",
            mime="text/csv",
        )
...
def main():
    ...
    st.caption(
        "Compares baseline surveys 815/816/817 with midline surveys 880/881/882. "
        "ECD is intentionally excluded from this first midline page."
    )
    ...
    matched = helpers.build_matched_assessment_pairs(filtered)
    st.divider()
    render_summary_metrics(filtered, matched)
    st.divider()
    render_phase_score_chart(filtered)
    st.divider()
    render_matched_gain_section(matched)
    st.divider()
    render_benchmark_section(filtered)
    st.divider()
    render_school_section(matched)
    st.divider()
    render_data_quality(filtered, matched)
    st.divider()
    render_midline_completion_section(filtered)
