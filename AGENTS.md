...
- For assessment counts/completion dashboards, do **not** filter on `assessment_complete` as proof of a valid saved assessment. TeamPact/2026 rows can have `assessment_complete = 0` while still being collected responses with scores; count response rows (and show distinct `participant_id` separately when useful).
